...
dayjs.extend(utc)
    ...
    try {
      ...
      return allBirthdays.map(el=> {
        return {
          ...
          formatedDate: ' ' + el.day + ' de ' + MONTHS_LABEL[el.month as keyof typeof MONTHS_LABEL],
          month:el.month,
          day:el.day
        }
      })
  ...
  export const addBirthday = async (message:string,author: string) => {
    try {
  ...
  export const parseBirhdayMessage = (message:String) => {
    try {
