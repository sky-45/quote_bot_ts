...
const { CLIENT_ID, CLIENT_SECRET } = process.env
  ...
  export const getChannel = async (channel:any, live_only=false) => {
    try {
      ...
      if(streams.length == 0) throw new Error('Canal no encontrado')
  ...
  export const getChannels = async (channels:any, live_only=false) => {
    try {
      const access_token = await getAccessToken()

      const live_info_channels = await Promise.all(channels.map((channel:string) => {
  ...
  export const addChannel = async (query:string, addedBy:string) => {
    try {
...
// 3. apagado -> prendido: stado redis = OFF
  ...
  export const notifyChannelsLive = async (channels:any = [], chat:any) => {
    try {
      const channelsPendingNotify:any = []
